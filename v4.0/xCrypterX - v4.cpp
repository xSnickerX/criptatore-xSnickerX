...
int main() {
	...
	do {
		...
		getline(cin, stin);									// ottengo la stringa de criptare
		if (stin.length() < 1) {
			cout << "String NULL. Enter a valid string." << endl;	//segnalo l'errore di stringa nulla
		}
	} while (stin.length() < 1);							//verifico che la stringa non sia nulla, se while e' true, richiede l'inserimento
	...
	*/
	...
	int stinL_pos = stin.length() + fbci_num;	
	char stinL = char(stinL_pos);					// assegna a stinL il carattere ascii corrispondente al dec della lunghezza stringa (stin.length() = 3 --> sinL = ETX)
